...
def test_client_data_isolation(client, db):
    # 1. Cria dois clientes distintos
    c1 = models.Cliente(id=uuid.uuid4(), nome="Cliente Alpha", ativo=True)
    c2 = models.Cliente(id=uuid.uuid4(), nome="Cliente Beta", ativo=True)
    db.add_all([c1, c2])
    db.commit()
    ...
    cap1 = models.MensagemCapturada(id=uuid.uuid4(), cliente_id=c1.id, sender_name="Lead Alpha", message_content="Captura Alpha", group_jid="alpha@g.us")
    ...
    cap2 = models.MensagemCapturada(id=uuid.uuid4(), cliente_id=c2.id, sender_name="Lead Beta", message_content="Captura Beta", group_jid="beta@g.us")

    db.add_all([g1, m1, cap1, g2, cap2])
    db.commit()
