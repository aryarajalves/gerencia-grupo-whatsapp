...
def listar_clientes(db: Session = Depends(get_db)):
    ...
    """
    clientes = db.query(models.Cliente).filter(models.Cliente.ativo == True).order_by(models.Cliente.criado_em.asc()).all()
    if not clientes:
        # Se não houver nenhum, cria o cliente padrão inicial
